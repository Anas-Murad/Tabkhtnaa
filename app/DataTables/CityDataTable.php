...
class CityDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('action', function ($row) {
                $btns =" <a href='".(route('admin.settings.cities.edit' , [$row->country_id , $row->id]))."' class='dropdown-item'><i class='ph-pencil me-2'></i>    تعديل</a>";
                $DeleteFunction = "DeleteFunction(`".route("admin.settings.cities.destroy", [$row->country_id , $row->id])."`)" ;
                $btns .=" <a href= '#'  onclick='".  $DeleteFunction . "'  class='dropdown-item'><i class='ph-trash me-2'></i> حذف </a>";
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('name')->title('اسم المدينه'),
            Column::make('iso2')->title('iso2'),
            Column::make('latitude')->title('latitude'),
            Column::make('longitude')->title('longitude'),
            Column::make('flag')->title('حاله المدينه'),
